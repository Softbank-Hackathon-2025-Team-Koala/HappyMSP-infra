...
########################################
# ECR 모듈 호출
# - 프로젝트 전용 컨테이너 레지스트리를 생성.
# - 마이크로서비스별 컨테이너 이미지를 저장.
########################################
module "ecr" {
  source       = "./modules/ecr"  # ECR 모듈 경로
  project_name = var.project_name # repo 이름 prefix
}
...
module "alb_controller" {
  source = "./modules/irsa-alb-controller" # ALB Controller 모듈 경로

  cluster_name     = module.eks.cluster_name     # EKS 클러스터 이름
  cluster_endpoint = module.eks.cluster_endpoint # 클러스터 엔드포인트 URL
